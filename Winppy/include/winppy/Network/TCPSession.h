...
namespace winppy
{
	...
	public:
		...
		char m_cancelIo;
		char m_isSending;
		uint16_t m_pending;	// I/O pending packet count (이 변수에 대한 접근은 isSending 플래그 인터락 매커니즘으로 단일 스레드만 변경을 보장해야 함.)
	private:
		uint64_t m_id;
		SOCKET m_sock;

		// alignas(Cache::L1LineSize()) WSAOVERLAPPED m_recvOverlapped	// Test 필요
